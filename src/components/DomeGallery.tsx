...
function buildItems(pool: ImageInput[], seg: number, isMobile: boolean = false): GridItem[] {
...
export default function DomeGallery({
  ...
  const activeOpenedWidth = isMobile ? "82vw" : openedImageWidth;
  const activeOpenedHeight = isMobile ? "110vw" : openedImageHeight;

  const activeSegments = isMobile ? 14 : segments;
  const activeDragSensitivity = isMobile ? 12 : dragSensitivity;
  ...
  const items = useMemo(() => buildItems(images, activeSegments, isMobile), [images, activeSegments, isMobile]);
  ...
  useEffect(() => {
    ...
    const ro = new ResizeObserver((entries) => {
      ...
      const activeMinRadius = isMobileLocal ? 320 : minRadius;
      radius = clamp(radius, activeMinRadius, maxRadius);
      ...
      root.style.setProperty("--perspective-factor", isMobileLocal ? "3.2" : "2");
      ...
      if (enlargedOverlay && frameRef.current && mainRef.current) {
        ...
        const hasCustomSize = activeOpenedWidth && activeOpenedHeight;
        if (hasCustomSize) {
          const tempDiv = document.createElement("div");
          tempDiv.style.cssText = `position: absolute; width: ${activeOpenedWidth}; height: ${activeOpenedHeight}; visibility: hidden;`;
  ...
  const startInertia = useCallback(
    (vx: number, vy: number) => {
      ...
      let frames = 0;
      const d = clamp(dragDampening ?? 2, 0, 10);
      const frictionMul = 0.94 - 0.01 * d;
      const stopThreshold = 0.015 + 0.005 * d;
      const maxFrames = Math.round(90 + 30 * d);
      const step = () => {
        vX *= frictionMul;
        vY *= frictionMul;
  ...
  const openItemFromElement = useCallback(
    (el: HTMLElement) => {
      ...
      const wantsResize = activeOpenedWidth || activeOpenedHeight;
      if (wantsResize) {
        const onFirstEnd = (ev: TransitionEvent) => {
          ...
          const tempWidth = activeOpenedWidth || `${frameR.width}px`;
          const tempHeight = activeOpenedHeight || `${frameR.height}px`;
    ...
    [enlargeTransitionMs, lockScroll, activeOpenedHeight, activeOpenedWidth, activeSegments, unlockScroll]
  );
