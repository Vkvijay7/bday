...
export default function OceanCanvas({ sliderVal }: OceanCanvasProps) {
  const canvasRef = useRef<HTMLCanvasElement | null>(null);
  const timeRef = useRef(sliderVal / 1000);
  const mouseXRef = useRef(0.5);

  // Sync prop changes to the time ref used by the drawing loop
  useEffect(() => {
    timeRef.current = sliderVal / 1000;
  }, [sliderVal]);
  ...
  useEffect(() => {
    ...
    const handleTouchMove = (e: TouchEvent) => {
      if (e.touches && e.touches.length > 0) {
        mouseXRef.current = e.touches[0].clientX / window.innerWidth;
      }
    };
    ...
    const draw = () => {
      if (disposed) return;
      T += 0.016;
      const P = getPalette(timeRef.current);

      /* sun position */
      const sunX = W * (0.5 + (mouseXRef.current - 0.5) * 0.25);
      ...
      const NUM = isMobile ? 12 : 26; // Reduce wave count on mobile to drop CPU workload
      const stepX = isMobile ? 18 : 6;  // Plot path coordinates less frequently on mobile
      for (let i = 0; i < NUM; i++) {
        ...
        if (depth > 0.62) {
          ...
          const foamStep = isMobile ? 24 : 9; // foam particles drawn less frequently on mobile
      ...
      /* ── SUN GLITTER PATH ── */
      const glitterCount = isMobile ? 70 : 220; // Lower particle draw load on mobile
  ...
  return (
    <canvas
      ref={canvasRef}
      id="ocean"
      className="fixed inset-0 w-screen h-screen z-40 pointer-events-none block"
    />
  );
}
